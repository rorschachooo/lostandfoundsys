...
const confirmDelBatch = () => {
  ...
  request.post('/recruit/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// delete
const del = (id) => {
  request.delete('/recruit/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('Operation successful')
      load()  // Refresh table data
    } else {
...
// Export Interface
const exportData = () => {
  window.open(`http://${config.serverUrl}/recruit/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('recruit.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> Add </span>
      </el-button>
      <el-upload
          v-if="auths.includes('recruit.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/recruit/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('recruit.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> Export </span>
      </el-button>
      <el-popconfirm title="are you sure you want to delete?" @confirm="confirmDelBatch" v-if="auths.includes('recruit.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column prop="address" label="Pickup Location"></el-table-column>
      <el-table-column prop="time" label="Pickup time"></el-table-column>
      <el-table-column label="Publisher"><template #default="scope"><span v-if="scope.row.user_id">{{ state.userOptions.find(v => v.id === scope.row.user_id) ? state.userOptions.find(v => v.id === scope.row.user_id).name : '' }}</span></template></el-table-column>
        ...
        <el-table-column label="operate" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('recruit.edit')">Revise<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="are you sure you want to delete?" @confirm="del(scope.row.id)" v-if="auths.includes('recruit.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="Lost and found information" width="40%">
    ...
    />
        ...
        <el-form-item prop="address" label="Pickup Location">
          <el-input v-model="state.form.address" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="time" label="Pickup time">
