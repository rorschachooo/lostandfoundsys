...
const confirmDelBatch = () => {
  ...
  request.post('/banner/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// Export interface
const exportData = () => {
  window.open(`http://${config.serverUrl}/banner/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('banner.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> Add </span>
      </el-button>
      <el-upload
          v-if="auths.includes('banner.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/banner/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('banner.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> Export </span>
      </el-button>
      <el-popconfirm title="Are you sure you want to delete?" @confirm="confirmDelBatch" v-if="auths.includes('banner.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column prop="id" label="Slideshow number"></el-table-column>
      <el-table-column label="picture"><template #default="scope"><el-image preview-teleported style="width: 120px; height: 80px" :src="scope.row.img" :preview-src-list="[scope.row.img]"></el-image></template></el-table-column>
      <el-table-column prop="url" label="Link address"></el-table-column>
      <el-table-column prop="index_radio" label="Is it home page"></el-table-column>

        <el-table-column label="operate" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('banner.edit')">Revise<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="Are you sure you want to delete?" @confirm="del(scope.row.id)" v-if="auths.includes('banner.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="Slideshow information" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item prop="img" label="picture">
        ...
        <el-form-item prop="url" label="Link address">
          <el-input v-model="state.form.url" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="indexRadio" label="Is it home page">
          <el-radio-group v-model="state.form.index_radio">
