...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-popconfirm title="are you sure you want to delete?" @confirm="confirmDelBatch" v-if="auths.includes('notice.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="Preview"><template #default="scope"><el-button @click="view(scope.row.content)">Check</el-button></template></el-table-column>
      <el-table-column label="Founder"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>

        <el-table-column label="operate" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('notice.edit')">Revise<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="are you sure you want to delete?" @confirm="del(scope.row.id)" v-if="auths.includes('notice.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="Announcement" width="40%">
    ...
    />
        ...
        <el-form-item prop="userId" label="Founder">
          <el-select clearable v-model="state.form.user_id" placeholder="Please select"  style="width: 100%">
