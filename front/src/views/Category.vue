...
const confirmDelBatch = () => {
  ...
  request.post('/category/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// Export Interface
const exportData = () => {
  window.open(`http://${config.serverUrl}/category/export`)
}
...
<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('category.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> Add </span>
      </el-button>
      <el-upload
          v-if="auths.includes('category.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/category/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('category.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> Export </span>
      </el-button>
      <el-popconfirm title="Are you sure you want to delete?" @confirm="confirmDelBatch" v-if="auths.includes('category.deleteBatch')">
        <template #reference>
      ...
      <el-table-column prop="name" label="Category Name"></el-table-column>
        ...
        <el-table-column label="operate" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('category.edit')">Revise<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="are you sure you want to delete?" @confirm="del(scope.row.id)" v-if="auths.includes('category.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="Item type information" width="40%">
